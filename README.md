...
Hardware & network inventory · employee handovers with printable PDF receipts · software licenses · mobile lines · vendors & contracts · repairs · physical stock counts · several legal entities under one install · an ITIL-aligned service desk (incidents, requests, approvals, SLAs) · a natural-language AI assistant · a full audit trail — all behind a built-in, mobile-ready web UI running entirely on your own infrastructure.
...
### 🏛 Several companies under one install
A group that runs more than one legal entity registers each of them in **Product Catalog → Companies**, optionally nested under a parent, each with its own logo, address, tax details and handover terms. Assets, employees, mobile lines, licenses, contracts, consumables and stock counts all carry an **owning company**; Hardware and Employees gain a Company column and filter. The **zimmet form is issued under the employee's own company** — its letterhead heads the page. Hand over a sister company's device and the form adds an **Owner Company** column plus a clause stating the equipment stays that entity's property; a third document option issues **one form per owner company**. Letterhead and per-line owner are **snapshotted onto the receipt**, so a reprint years later still matches the signed original. Reports gain a **company scope selector** (presets + KPI tiles) and two company reports. Every field is optional and falls back to the workspace values in Settings, so a **single-company install is untouched** — no extra column, filter or picker.
...
| **Product Catalog** | Approved brands/models (with **per-model EOL lifecycle**), categories, locations, departments, spec options & **Companies** (legal entities with their own letterhead) |
...
| **Reports** | 22 preset reports + a builder (data sources × columns × filters), **company scope selector**, CSV / letterhead print |
...
| **Service Desk** | Incidents & requests — Impact×Urgency priority, SLA timers, workflow editor + auto-close, **automation rules** (auto-categorise / prioritise / assign on creation), canned replies, CSAT, three-level note visibility, VIP requesters, two-way email-to-ticket (IMAP, password or OAuth2) |
...
| GET/POST/PATCH/DELETE | `/api/companies` · `/options` · `/:id` · `/:id/default` | settings:manage (`/options`: any) | **Companies** — legal entities with their own branding, address and handover terms; `options` feeds the pickers for every caller |
| GET/PUT | `/api/integrations/inbound-mail` · `/test` · `/poll` · `/blocklist` · `/release` | integration:read / manage | Email-to-ticket (IMAP) — config (password encrypted, masked), test connection, manual poll, blocked senders + bulk-mail filter, re-open a filtered message anyway |
| GET/PUT/POST | `/api/integrations/mail-oauth/apps` · `/status` · `/start` · `/callback` · `/disconnect` | integration:read / manage (`callback`: state-bound) | **Connected mailbox (OAuth2)** — register the Microsoft / Google app once, sign in, keep an encrypted refresh token that serves both IMAP and SMTP |
