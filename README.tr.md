...
Envanterinizin yanında yaşayan eksiksiz bir ITSM paketi. **Kayıtlar (incident) ve talepler** — **Etki × Aciliyet** ile türetilen öncelik, 7/24 çalışan **SLA** yanıt/çözüm sayaçları (*beklemede* durumunda duraklatma ve ihlal işareti), **Jira tarzı iş akışı editörü** (özel durum geçişleri + *çözülenleri otomatik kapat*), kayıt açılır açılmaz triyajı yapan **otomasyon kuralları** — *konu "toner" içeriyor ve e-postadan geldiyse kategoriyi ata, önceliği düşür, baskı ekibine ver* — kuralı kaydetmeden deneyen test paneli ve kural başına eşleşme sayacı ile, hazır yanıtlar ve kapanışta **CSAT** anketi. **Talep şablonları** self-service **Portal**'ı besler — personelin zimmetini gördüğü aynı giriş — **çok adımlı onay zincirleri** (yönetici → BT ekibi → son onay), vekâlet, hatırlatma ve eskalasyon ile. Org şemasının doğru cevabı vermediği durumlarda — yöneticinin izinli olduğu bir hafta, raporlama hattı dışındaki bir bütçe sahibi — tek bir kayıt **adıyla seçilen bir onaylayıcıya** da gönderilebilir; kimse kaydı kendi kendine onaylatamaz. Yorum ve eklerde **üç seviyeli görünürlük** — *herkese açık*, *yalnızca onaycı*, *yalnızca BT* — fiyat araştırması ve satınalma detaylarını talep sahibinden gizler, onaycıya ise gerekeni gösterir. Ayrıca **Problem** ve **Değişiklik** yönetimi, portal yönlendirmeli **Bilgi Bankası**, geçmiş kayıtların nasıl çözüldüğünü gösteren **"benzer eski kayıtlar"** paneli ve **çift yönlü e-postadan kayıt açma**: gelen posta **DMARC doğrulamalı** gönderici eşlemesiyle kayda dönüşür, personelin yanıtı düzenlenebilir bir *Servis masası — yanıt* şablonuyla ve konuda `[REQ-1234]` taşıyarak geri gider, talep sahibinin cevabı da ekleriyle birlikte kayda herkese açık yorum olarak düşer. Posta kutusu parolayla ya da **Microsoft 365 / Google'a oturum açarak (OAuth2)** bağlanır. **VIP talep sahipleri** — çalışan kartındaki bir işaret — sabitlenmiş bir öncelik yerine yükseltilmiş **aciliyetle** kayıt açar; böylece operatörün kendi Etki × Aciliyet seçimi hiçbir zaman ezilmez.
...
### 🏛 Tek kurulumda birden fazla firma
Birden fazla tüzel kişilik işleten bir grup, her şirketi **Ürün Kataloğu → Firmalar** altında — isterse bir üst firmaya bağlı olarak — kendi logosu, adresi, vergi bilgileri ve zimmet şartlarıyla tanımlar. Cihazlar, çalışanlar, hatlar, lisanslar, sözleşmeler, sarf malzemeleri ve sayımlar bir **sahip firma** taşır; Donanım ve Çalışanlar listelerine Firma sütunu ve filtresi gelir. **Zimmet tutanağı çalışanın kendi firması adına düzenlenir** — sayfanın başında o firmanın anteti durur. Kardeş şirkete ait bir cihaz yine aynı çalışana zimmetlenebilir: tutanak o satırlar için **Sahip Firma** sütunu ve cihazın ilgili grup şirketinin mülkiyetinde kaldığını belirten bir kloz ekler; üçüncü bir belge seçeneği ise **her sahip firma için ayrı tutanak** üretir. Antet ve satır bazındaki sahiplik tutanağa **anlık kopyalanır**, böylece yıllar sonra alınan bir çıktı imzalı aslıyla birebir aynı kalır. Raporlara her hazır rapora ve KPI kartlarına uygulanan bir **firma kapsam seçici** ile iki yeni firma raporu eklenir. Tüm alanlar opsiyoneldir ve Ayarlar'daki değerlere geri düşer; **tek firmalı kurulumda hiçbir şey değişmez** — ne ek sütun, ne filtre, ne seçici.

### 🤖 AI asistanı (doğal dil sorguları)
Envanterinizi düz cümleyle sorun, veriye dayalı cevap alın — **sağlayıcıdan bağımsız**: yerel bir **Ollama** modeli ya da bir bulut API'si (OpenAI, DeepSeek, Anthropic, Groq, Mistral, Together, OpenRouter veya kendi uç noktanız). Akışlı yanıtlar, sonuç tabloları, CSV dışa aktarım, otomatik grafikler ve açılır bir "SQL'i göster". Analitik sorular, temel tablolara değil **derlenmiş bir view şeması** üzerine, düşük yetkili bir rol altında **korumalı salt-okunur** sorgu olarak çalışır; her kullanıcının RBAC'ini gözetir (arayüzün göstermediği veriyi asistan da göstermez), dışarı çıkan istekler SSRF korumalıdır ve kullanıcı başına hız sınırı vardır. **Varsayılan olarak kapalıdır** — **Entegrasyonlar → AI** altından açılır.

### 🔐 Rol bazlı erişim kontrolü + izin matrisi
Altı yerleşik rol — `Owner`, `Admin`, `Helpdesk`, `Viewer`, ayrıca **`HR`** (onboarding/offboarding *talebi* oluşturur, yalnızca kendi zimmetini görür) ve **`Portal`** (self-service çalışan girişi, yalnızca kendi cihazlarını görür) — **her** uç noktada uygulanır ve her istekte yeniden denetlenir; değişiklikler anında geçerli olur. Rolden daha ince kontrol mü lazım? IAM matrisinde **özel izin grubu** oluşturun: herhangi bir `resource:action` (ör. `asset:sell`, `license:assign`) verin ve departman / lokasyon / kategori / maliyet limiti ile kısıtlayın. Sahipler hesapları devre dışı bırakabilir veya silebilir — her pasifleştirme/aktifleştirme/silme/rol değişikliği kaydedilir. Giriş yerel e-posta/parola ile; **TOTP MFA** her rol için opsiyonel, **`Owner` hesapları için zorunludur**: bir Owner uygulamayı kullanmadan önce MFA'yı etkinleştirmek zorundadır, kapatamaz ve MFA'sı olmayan hiç kimse Owner'a yükseltilemez. Ayrıca parola değişikliği ve sunucu taraflı çıkış (JWT iptali). **OpenID Connect ile çoklu oturum açma (SSO)** de vardır — davet usulü, varsayılan olarak kapalı: Google Workspace / Microsoft Entra / Okta / Auth0 / Keycloak ile giriş yapılır, PKCE'li Authorization Code akışı kullanılır, hesap **oluşturmaz** (yalnızca ITACM'de zaten var olan, e-postası **doğrulanmış** kullanıcıyı tanır) ve yerel parola girişi acil durum yolu olarak açık kalır. Arayüzden yapılandırılır: **Entegrasyonlar → SSO**. **Active Directory / LDAP** entegrasyonu da vardır: personel dizin parolasıyla giriş yapabilir (yine davet usulü), çalışan listesi ile ünvan/departman/yönetici bilgisi dizinden senkronize edilir — yönetici bilgisi onay zincirini kendiliğinden besler — ve **grup → rol** eşlemesiyle BT hesapları açılabilir (`Owner` bu yolla asla verilmez). Senkronize edilen çalışanlara **self-servis portal girişi** de açtırılabilir: geçici parola üretilmez, kimseden parola değiştirmesi istenmez, personel doğrudan **dizin parolasıyla** girer. Dizin hesabının yerel parolası olmadığı için hesap ekranı parola formu yerine parolanın nerede değiştirileceğini yazar ve API isteği baştan reddeder — aksi hâlde dizin hesabına yerel parola konabilir ve kişi AD'de kapatılsa bile giriş yapmaya devam edebilirdi. Her senkron çalıştırması açtığı, rolünü değiştirdiği ve kapattığı hesapları **adıyla** denetim kaydına yazar; gece çalışan bir senkrondan sonra kimin erişim kazandığını görmek için kullanıcı listesini elle karşılaştırmak gerekmez. Ayrılanları pasife alma işlemi, tek çalıştırmada senkronize çalışanların %30'undan fazlası eksikse otomatik durur. Kuru çalıştırma (önizleme), saatlik/günlük zamanlama ve bağlantı testi arayüzde: **Entegrasyonlar → Dizin**.
...
Kenar menü, özellik setiyle bire bir eşleşir; üstünde her ekranda duran bir **AI asistanı** (⌘/Ctrl+J) vardır:

| Modül | Ne yapar |
|---|---|
| **AI Asistanı** | Envanter üzerinde doğal dil sorguları — yüzen başlatıcı (⌘/Ctrl+J), sağlayıcıdan bağımsız, korumalı salt-okunur. Varsayılan kapalı (Entegrasyonlar → AI) |
...
| **Ürün Kataloğu** | Onaylı marka/model (**model-bazlı EOL yaşam döngüsü** ile), kategori, lokasyon, departman, özellik seçenekleri & **Firmalar** (kendi antetli tüzel kişilikler) |
...
| **Raporlar** | 22 hazır rapor + oluşturucu (kaynak × sütun × filtre), **firma kapsam seçici**, CSV / antetli yazdırma |
...
| **Servis Masası** | Kayıtlar & talepler — Etki×Aciliyet önceliği, SLA sayaçları, iş akışı editörü + otomatik kapatma, hazır yanıtlar, CSAT, üç seviyeli not görünürlüğü, VIP talep sahipleri, çift yönlü e-postadan kayıt açma (IMAP, parola veya OAuth2) |
...
| POST | `/api/ai/query` | personel | **AI asistanı** — SSE akışı, araç döngüsü, korumalı salt-okunur sorgular (kullanıcı başına hız sınırlı) |
| GET | `/api/ai/status` | personel | Asistanın durumu (sağlayıcı/model) |
| GET/PUT/DELETE | `/api/ai/config` | integration:read / manage | AI ayarlarını oku / kaydet / temizle (API anahtarı şifreli, maskeli) |
...
| GET/POST/PATCH/DELETE | `/api/companies` · `/options` · `/:id` · `/:id/default` | settings:manage (`/options`: tümü) | **Firmalar** — kendi antet, adres ve zimmet şartlarına sahip tüzel kişilikler; `options` tüm ekranlardaki seçicileri besler |
| GET/PUT | `/api/integrations/inbound-mail` · `/test` · `/poll` · `/blocklist` · `/release` | integration:read / manage | E-postadan kayıt açma (IMAP) — yapılandırma (parola şifreli, maskeli), bağlantı testi, elle çekme, engellenen göndericiler + toplu posta filtresi, filtrenin atladığı bir postayı yine de kayda çevirme |
| GET/PUT/POST | `/api/integrations/mail-oauth/apps` · `/status` · `/start` · `/callback` · `/disconnect` | integration:read / manage (`callback`: state'e bağlı) | **Bağlı posta kutusu (OAuth2)** — Microsoft / Google uygulamasını bir kez tanımla, oturum aç, hem IMAP hem SMTP'ye hizmet eden şifreli bir yenileme jetonu sakla |
...
- **Servis masası sınırları:** Portal kullanıcıları yol düzeyinde `/api/me/*` ile sınırlıdır ve her okuma kendi personel kaydına daraltılır; kayıt yorumları ve ekleri üç görünürlük seviyesi taşır (**herkese açık / yalnızca onaycı / yalnızca BT**) ve bu, her okuma yolunun SQL `WHERE` cümlesinde uygulanır — dahili notlar talep sahibine, yalnızca-BT notları onaycıya asla ulaşmaz. **E-postadan kayıt açma**, talep sahibini (ve `[REQ-N]` çapraz bağını) yalnızca sağlayıcı tarafından doğrulanmış bir **DMARC geçişinde** eşleştirir; sahte bir `From` başkasının adına kayıt açamaz. E-postayla gelen bir yanıt ise kayda yalnızca gönderen o kaydın talep sahibiyse (veya DMARC ile doğrulanmışsa) eklenir; kimse kayıt numarasını tahmin ederek içeri yorum düşemez. Bağlı posta kutularının OAuth yenileme jetonları şifreli saklanır ve otomatik döndürülür; bağlantı akışı `state` değerini tarayıcıya bağlar ve JWT algoritmasını sabitler.
